...
def get_thread_messages(client, thread_id: str) -> list:
    ...
    for msg in response:
        messages.append(
            {"role": msg.role, "content": msg.content[0].text.value if msg.content else ""}
        )
    return messages
...
def main():
    ...
    print("\n" + "=" * 80)
    print("🔄 CONFIG VERSION CHECK")
    print(f"   Compiled At: {compiled_at}")
    print(f"   Version: {version}")
    print("   (Compare this timestamp with your local compile time to verify sync)")
    print("=" * 80)
